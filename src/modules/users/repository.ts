...
class UsersRepository extends BaseRepository {
  async findAll() {
    return this.dbAccess!("user").returning("*");
  }

  async findById(id: string): Promise<User | undefined> {
    return this.dbAccess<User>("user").where({ id }).first();
  }

  async _findByCond(cond: object): Promise<User | undefined> {
    return this.dbAccess<User>("user")
      .leftJoin("user_refresh_tokens", "user_refresh_tokens.user_id", "user.id")
      .select(
        "user.id",
        "user.user_name",
        "user.email",
  ...
  async createUserWithToken(
    ...
    const [newUser] = await this.dbAccess("user")
      .insert([
        {
  ...
  async createUser(userData: UserInputData): Promise<User[]> {
    const [newUser] = await this.dbAccess("user")
      .insert([
        {
          user_name: userData.userName,
